...
def btpiter_ind(iter,btpobj,X):
    ...
    return out1.rx2('statistic')[0]
def btpiter_CI(iter,btpobj,X,Z):
    from rpy2.robjects.packages import importr
    d = {'print.me': 'print_dot_me', 'print_me': 'print_uscore_me'}
    kpcalg = importr('kpcalg', robject_translations = d)
    Y1all = btpobj
...
def hsic_CI(X,Y,Z=None,alpha=0.05,reps=50):
    ...
    if Z is None:
        ...
        #A_fin_rem.append(pool.map(partial(calc_A.remote,data=data[(method_name,alpha,noise)],method_name=method_name,lag=lag,alpha=alpha),range(25)))
        btpobj = Y1all
        nullapprox = pool.map(partial(btpiter_ind.remote,btpobj=btpobj,X=X),range(50))
        ...
        btpobj = Y1all
        nullapprox = pool.map(partial(btpiter_CI.remote,btpobj=btpobj,X=X,Z=Z),range(50))
