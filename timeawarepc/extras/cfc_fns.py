def val_pc_gauss(data_trans,alpha):
    from timeawarepc.pcalg import estimate_skeleton, estimate_cpdag, causaleff_ida, ci_test_gauss
...
def val_pc_hsic_btp(data_trans,alpha):
    from timeawarepc.pcalg import estimate_skeleton, estimate_cpdag, hsic_condind, causaleff_ida, ci_test_gauss
    import networkx as nx
    (g, sep_set) = estimate_skeleton(indep_test_func=hsic_condind,
                                        data_matrix=data_trans,
                                        alpha=alpha,method='stable')
...
def val_pc_gauss_btp(data_trans,alpha):
    from timeawarepc.pcalg import estimate_skeleton, estimate_cpdag, causaleff_ida, ci_test_gauss, ci_test_gauss_btp
    import networkx as nx
...
def val_neuropc_lin(data,lag=1,subsampsize=50,n_iter=1,alpha=0.3,thresh=0.25,v2=False):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed, data_transformed_overlapping, causaleff_ida, return_finaledges, estimate_cpdag, estimate_skeleton,ci_test_gauss, orient
    #import rpy2.robjects as robjects
    #from rpy2.robjects.packages import importr
    #import rpy2.rlike.container as rlc
    #from rpy2.robjects import pandas2ri
...
def val_tpc_sno(data,lag=1,alpha=0.1):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_overlapping,data_transformed,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges
    import random
    import networkx as nx
    start_time = time.time()
...
def val_tpc_sno_nogaps(data,lag=1,alpha=0.1):
    #random.seed(111)
    import time
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_nogaps,data_transformed,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges
    import random
    import networkx as nx
    start_time = time.time()
    data_trans = data_transformed_nogaps(data, lag-1)
    (g, sep_set) = estimate_skeleton(indep_test_func=ci_test_gauss,
                                        data_matrix=data_trans,
                                        alpha=alpha,method='stable')
    g = estimate_cpdag(skel_graph=g, sep_set=sep_set)
...
#    g=orient(g,lag,data.shape[1])
    ...
    print(time.time()-start_time)
    return val_out, causaleffin
def val_tpc_sno_mod(data,lag=1,alpha=0.1):
    #random.seed(111)
    import time
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_overlapping,data_transformed_mod,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges
    import random
    import networkx as nx
    start_time = time.time()
    p = data.shape[1]
    ...
    causaleff1={}
    for i in range(p):
        data_trans = data_transformed_mod(data, lag-1,i)
        (g1[i], sep_set) = estimate_skeleton(indep_test_func=ci_test_gauss,
                                            data_matrix=data_trans,
                                            alpha=alpha,method='stable')
        g1[i] = estimate_cpdag(skel_graph=g1[i], sep_set=sep_set)
        causaleff1[i] = causaleff_ida(g1[i],data_trans)
        lag1=lag
        mapping = {0:lag1*i}
        for i1 in range(p):
            for j1 in range(1,lag1):
                mapping[(lag1-1)*i1+j1]=lag1*i1+j1
        #print(g1)
        g2[i]=nx.relabel_nodes(g1[i],mapping)
    nodeset = range(p*lag1)
    g = nx.DiGraph()
    g.add_nodes_from(nodeset)
    for i1 in range(p):
            for i2 in range(p):
                    if (0,i2+1) in list(g1[i1].edges()):
                            g.add_edge(2*i1,2*i2+1)
    causaleff2 = np.zeros((p*lag1,p*lag1))
    for i1 in range(p):
        for j1 in range(lag1):
            for i2 in range(p):
                for j2 in range(lag1):
                    if (lag1*i1+j1,lag1*i2+j1) in list(g.edges()):
                        if j1==0 and j2>=1:
                            causaleff2[lag1*i1+j1,lag1*i2+j2] = causaleff1[i1][0,(lag1-1)*i2+j2]
                        else:
                            causaleff2[lag1*i1+j1,lag1*i2+j2] = causaleff1[i1][(lag1-1)*i1+j1,(lag1-1)*i2+j2]
    G,causaleffin, causaleffin2=return_finaledges_v2(g,causaleff2,lag,data.shape[1])
#    g=orient(g,lag,data.shape[1])
    val_out=G#nx.adjacency_matrix(G).toarray()
    print(time.time()-start_time)
    return val_out, causaleffin
def val_tpc_sno_hsic_btp(data,lag=1,alpha=0.1):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_overlapping,hsic_condind,data_transformed,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges
    import random
    import networkx as nx
    start_time = time.time()
    data_trans = data_transformed(data, lag-1)
...
def val_tpc_sno_btp(data,lag=1,alpha=0.1):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_overlapping,data_transformed,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges, ci_test_gauss_btp
    import random
    import networkx as nx
    start_time = time.time()
    data_trans = data_transformed(data, lag-1)
...
def val_tpc_sno_btstrp(data,lag=1,alpha=0.1):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed_btstrp,data_transformed,return_finaledges_v2 ,estimate_cpdag, estimate_skeleton, ci_test_gauss,causaleff_ida, return_finaledges
    import random
    import networkx as nx
    start_time = time.time()
...
def val_tpc_ns(data,lag=1,subsampsize=50,n_iter=50,alpha=0.3,thresh=0.25):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed,estimate_skeleton,return_finaledges_v2,estimate_cpdag,ci_test_gauss,data_transformed_overlapping, causaleff_ida, return_finaledges, orient
    import rpy2.robjects as robjects
    from rpy2.robjects.packages import importr
    import rpy2.rlike.container as rlc
    from rpy2.robjects import pandas2ri
...
def val_tpc_ns_hsic_btp(data,lag=1,subsampsize=50,n_iter=50,alpha=0.3,thresh=0.25):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed,hsic_condind,estimate_skeleton,return_finaledges_v2,estimate_cpdag,ci_test_gauss,data_transformed_overlapping, causaleff_ida, return_finaledges, orient
    import rpy2.robjects as robjects
    from rpy2.robjects.packages import importr
    import rpy2.rlike.container as rlc
    from rpy2.robjects import pandas2ri
...
def val_tpc_ns_btp(data,lag=1,subsampsize=50,n_iter=50,alpha=0.3,thresh=0.25):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed,estimate_skeleton,return_finaledges_v2,estimate_cpdag,ci_test_gauss,data_transformed_overlapping, causaleff_ida, return_finaledges, orient,ci_test_gauss_btp
    import rpy2.robjects as robjects
    from rpy2.robjects.packages import importr
    import rpy2.rlike.container as rlc
    from rpy2.robjects import pandas2ri
...
def val_neuropc(data,lag=1,subsampsize=50,n_iter=1,alpha=0.3,thresh=0.25,v2=False):
    ...
    import numpy as np
    import pandas as pd
    from timeawarepc.pcalg import data_transformed,data_transformed_overlapping, causaleff_ida, return_finaledges, orient
    import rpy2.robjects as robjects
    from rpy2.robjects.packages import importr
    import rpy2.rlike.container as rlc
    from rpy2.robjects import pandas2ri
...
def val_gc2(data,alpha):
    ...
    gcpack=importr('bruceR')
    vars = importr('vars')
    datapd=pd.DataFrame(data,columns=range(data.shape[1]))
    pandas2ri.activate()
    df = robjects.conversion.py2rpy(datapd)
    vm = vars.VAR(df,1)
    trap = io.StringIO()
    with redirect_stderr(trap):
        with redirect_stdout(trap):
            grangerout=gcpack.granger_causality(vm)
    goutpy=robjects.conversion.rpy2py(grangerout.rx2('result'))
    p=data.shape[1]
    adj_gc = np.zeros((p,p))
    for i in range(p):
        for j in range(p):
            if j<p-1 and goutpy.iloc[p*i+j,9]<=alpha:
                adj_gc[j,i] = 1
            # elif j==p-1 and goutpy.iloc[p*i+j,9]<0.001:
            #     adj_gc[:,i] = 1
    return adj_gc
def val_gc1(data,lag,alpha):
    #from scipy.stats import chi2
    from mvgc import tsdata_to_autocov, autocov_to_mvgc
    import numpy as np
    autocov = tsdata_to_autocov(data.T,lag)
    gc = autocov_to_mvgc(autocov,1,2)
    adj_mat = np.zeros((data.shape[1],data.shape[1]))
